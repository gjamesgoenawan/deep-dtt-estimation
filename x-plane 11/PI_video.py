...
class PythonInterface:
    def __init__(self):
        ...
        a = xp.worldToLocal(
            self.camera_static_lat[0], self.camera_static_lon[0], self.camera_static_alt[0])
        b = xp.worldToLocal(
            self.camera_static_lat[1], self.camera_static_lon[1], self.camera_static_alt[1])
    ...
    def XPluginStart(self):
        ...
        self.HotKey_0 = xp.registerHotKey(
            xp.VK_F8, xp.DownFlag, "Trigger Static External View 2", self.CameraHotKeyCallback_2, 0)
        self.HotKey_1 = xp.registerHotKey(
            xp.VK_F7, xp.DownFlag, "Trigger Static External View 1", self.CameraHotKeyCallback_1, 0)
        self.HotKey_2 = xp.registerHotKey(
            xp.VK_F6, xp.DownFlag, "Toggle Logging Action", self.LoggingHotKeyCallback, 0)
        self.HotKey_3 = xp.registerHotKey(
            xp.VK_F4, xp.DownFlag, "Toggle Monitoring Window", self.MonitoringHotkeyCallback, 0)
    ...
    def MoveFileCallback(self, elapsedMe, elapsedSim, counter, refcon):
        data_count = len(os.listdir(
            "C:\\Users\\gabriel\\Desktop\\aircraft-detection\\generated_data"))
        while True:
            try:
                os.mkdir(
                    f'C:\\Users\\gabriel\\Desktop\\aircraft-detection\\generated_data\\{data_count}')
                break
            except:
                data_count += 1
        ...
        camera_x, camera_y, camera_z = xp.localToWorld(xp.getDataf(
            self.camera_x), xp.getDataf(self.camera_y), xp.getDataf(self.camera_z))

        with open(f'C:\\Users\\gabriel\\Desktop\\aircraft-detection\\generated_data\\{data_count}\\camera.txt', 'w') as f:
            f.write(
                f'{camera_x}, {camera_y}, {camera_z}, {camera_pitch}, {camera_roll}, {camera_heading}')

        for i in os.listdir("C:\\X-Plane 11\\Output"):
            if '.avi' in i:
                os.rename(
                    f"C:\\X-Plane 11\\Output\\{i}", f"C:\\Users\\gabriel\\Desktop\\aircraft-detection\\generated_data\\{data_count}\\video.avi")
                break
        for i in os.listdir("C:\\X-Plane 11\\Output"):
            if '.avi' in i:
                os.remove(f'C:\\X-Plane 11\\Output\\{i}')
    ...
    def LoggingHotKeyCallback(self, inRefcon):
        if self.currently_recording == False:
            ...
            xp.registerFlightLoopCallback(
                self.LoggingFlightLoopCallback, -1, 0)
    ...
    def LoggingFlightLoopCallback(self, elapsedMe, elapsedSim, counter, refcon):
        ...
        if self.current_el < 11 and len(self.locs) > 3:
            ...
            past_three_elevation = [self.locs[-1][2],
                                    self.locs[-2][2], self.locs[-3][2]]
    ...
    def CameraHotKeyCallback_1(self, inRefcon):
        ...
        xp.controlCamera(xp.ControlCameraUntilViewChanges,
                         self.StaticCamera_1, 0)
    ...
    def DrawWindowCallback(self, inWindowID, inRefcon):
        ...
        if self.currently_recording:
            xp.drawString(color, left + 5, top - 20,
                          f'LOGGING MODE (LATLONEL = plane)', 0, xp.Font_Basic)
        else:
            a = xp.localToWorld(xp.getDataf(self.camera_x), xp.getDataf(
                self.camera_y), xp.getDataf(self.camera_z))
            self.current_lat = a[0]
            self.current_lon = a[1]
            self.current_el = a[2]
            xp.drawString(color, left + 5, top - 20,
                          f'MONITORING MODE (LATLONEL = camera)', 0, xp.Font_Basic)
        xp.drawString(color, left + 5, top - 40,
                      f'Latitude : {self.current_lat}', 0, xp.Font_Basic)
        xp.drawString(color, left + 5, top - 60,
                      f'Longitude: {self.current_lon}', 0, xp.Font_Basic)
        xp.drawString(color, left + 5, top - 80,
                      f'Elevation: {self.current_el}', 0, xp.Font_Basic)
        xp.drawString(color, left + 5, top - 100,
                      f'Plane Distance  : {geopy.distance.geodesic((1.3541354199301814, 103.97961848373048), (xp.getDataf(self.plane_lat), xp.getDataf(self.plane_lon))).nm}', 0, xp.Font_Basic)
